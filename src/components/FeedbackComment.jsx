...
export function FeedbackComment() {
  ...
  useEffect(() => {
    const handleToggle = () => {
      ...
      document.body.style.overflow = newIsActive ? 'hidden' : '';
      document.documentElement.style.overflow = newIsActive ? 'hidden' : '';
    };
  ...
  return (
    <>
      {isActive && (
        <>
          <div
            className="fixed inset-0 z-[90] bg-black/5 transition-colors duration-300"
            onClick={handleClick}
          ...
          <button
            ...
            className="fixed right-4 top-4 z-[100] flex items-center gap-1.5 rounded-full bg-white/90 px-4 py-2 text-sm font-medium text-gray-700 shadow-lg transition-all hover:bg-white hover:text-gray-900 dark:bg-zinc-800/90 dark:text-gray-300 dark:hover:bg-zinc-800 dark:hover:text-gray-100"
          >
      ...
      {showForm && (
        <div
          ref={formRef}
          className="fixed z-[100] w-96 rounded-lg border bg-white p-6 shadow-xl dark:bg-zinc-900 animate-zoom-in"
          style={{
